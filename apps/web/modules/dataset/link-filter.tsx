...
export default function LinkFilter() {
  ...
  useEffect(() => {
    setLinkFilters(
      datasetQuery?.dataset_link_filter.map(dataset_link_filter =>
        linkEntityToValue(
  ...
  }, [datasetQuery?.dataset_link_filter]);
        ...
        dataview_column!dataview_column_parent_dataview_id_fkey(
          ...
          role_column(
            ...
            role_link!role_link_configuration_schema_source_view_name_source_col_fkey(
              *,
              role_view!role_link_configuration_schema_target_view_name_fkey(
                name,
                pgt_pk_cols
              )
            )
  ...
  const { mutate: upsertLinkFilter } = useUpsertMutation(
    postgrest.schema(environment_schema).from('dataset_link_filter'),
    [
      'dataset_id',
      'source_view_name',
      'source_column_name',
      ...
      .then(() => {
        return upsertLinkFilter(
          values.map(linkFilterJson => {
            ...
            while (dataview?.id !== datasetQuery?.dataview_id) {
                ...
                .filter(
                  role_link =>
                    ...
                    role_link.role_view?.pgt_pk_cols?.includes(
                      // must point to a pk only pk's get added to page/layout context so only pks can be used to filterby)
