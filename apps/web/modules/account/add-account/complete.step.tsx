...
export function CompleteStep() {
  ...
  const { data, loading } = useHarvestEvalResultQuery({
    variables: { filters: { minPAndL: 0, excludeAssetSymbols: [], purchaseDateBefore: null, purchaseDateAfter: null } }
  });
  ...
  useEffect(() => {
    if (data?.harvestEvalResult) {
      const timer = setTimeout(() => {
        ...
        const totalSavings = opportunities.reduce((total, opp) => total + opp.taxSavings, 0);
    ...
    return (
      <div className="p-8">
        <div className="py-8 text-center">
          <h2 className="mb-4 text-xl font-semibold">Calculating Tax Opportunities...</h2>
        ...
        {opportunities.length > 0 && (
          <div className="mb-8">
            <h3 className="mb-4 text-left text-lg font-semibold">
              {data?.harvestEvalResult.harvestType === HarvestType.ReduceCostBasis 
                ? 'Cost Basis Reset Opportunities:'
                : data?.harvestEvalResult.harvestType === HarvestType.ReduceTaxes
                ...
                >
                  <div className="text-left">
                    ...
                    <div className={cn(
                      ...
                      {Format.money(opportunity.potentialLoss)} {opportunity.potentialLoss < 0 ? 'potential loss' : 'potential gain'}
                    </div>
                  </div>
