									...
									<div className="space-y-6">
										{(() => {
											...
											return filteredMerges.map((assetMerge) => {
												...
												const hasErrors =
													assetMerge.error || assetMerge.mergeError;
												...
												const totalShortTermPL =
													activeLotChangeList?.LotChange?.reduce(
														(sum, change) =>
															sum +
															Number(
																change.realizedProfitAndLossShortTerm || 0,
															),
														0,
													) || 0;
																			...
																			(lotChangeList, optionIndex) => {
																				...
																				const valueDeltaPercent =
																					targetValue !== 0
																						...
																						: 0;
																				const isApplied =
																					lotChangeList.appliedToAccount ===
																					true;
							...
							<div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
									...
									<p
										className={`text-2xl font-bold ${
											pAndLHistory
												.filter(
													(h) =>
														h.profitAndLossType === 'SHORT_TERM_CAPITAL_GAIN',
												)
								...
								<div className="bg-muted/50 rounded-lg p-4">
									...
									<p
										className={`text-2xl font-bold ${
											pAndLHistory
												.filter(
													(h) =>
														h.profitAndLossType === 'LONG_TERM_CAPITAL_GAIN',
												)
												.reduce((sum, h) => sum + Number(h.value || 0), 0) > 0
												? 'text-green-600'
												: pAndLHistory
									...
									>
										...
										{pAndLHistory
											.filter(
												(h) => h.profitAndLossType === 'LONG_TERM_CAPITAL_GAIN',
											)
											.reduce((sum, h) => sum + Number(h.value || 0), 0)
											.toFixed(2)}
									</p>
							...
							{pAndLHistory.map((history) => (
								<Card key={history.id} className="overflow-hidden">
									...
									<CardContent className="pt-6 space-y-6">
												...
												<div className="bg-muted/50 rounded-lg p-4">
													<div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
														...
														<div>
															...
															<p className="text-sm font-medium">
																{Number(
																	history.transaction.quantity || 0,
																).toFixed(4)}
															</p>
														</div>
														<div>
															...
															<p className="text-sm font-medium">
																$
																{Number(history.transaction.price || 0).toFixed(
																	4,
																)}
															</p>
														</div>
										...
										history.lotChangeList.LotChange &&
										history.lotChangeList.LotChange.length > 0 ? (
											<div className="space-y-3">
												<h4 className="font-medium text-sm flex items-center gap-2">
												...
												<div className="overflow-x-auto">
													<Table>
														...
														<TableBody>
															{history.lotChangeList.LotChange.map(
																(lotChange) => {
																	const costBasis =
																		Number(lotChange.price || 0) *
																		Number(lotChange.quantityFinal || 0);
																	return (
																		<TableRow
																			key={lotChange.id}
																			className="text-xs"
																		>
																			...
																			<TableCell className="text-right">
																				$
																				{Number(lotChange.price || 0).toFixed(
																					4,
																				)}
																			</TableCell>
																			<TableCell className="text-right font-medium">
																				{Number(
																					lotChange.quantityChange || 0,
																				).toFixed(4)}
																			</TableCell>
																			<TableCell className="text-right font-medium">
																				{Number(
																					lotChange.quantityFinal || 0,
																				).toFixed(4)}
																			</TableCell>
																			<TableCell className="text-right font-medium">
																				${costBasis.toFixed(2)}
																			...
																			<TableCell
																				className={`text-right font-medium ${
																					Number(
																						lotChange.realizedProfitAndLossShortTerm ||
																							0,
																					) > 0
																						? 'text-green-600'
																						: Number(
																			...
																			<TableCell
																				className={`text-right font-medium ${
																					Number(
																						lotChange.realizedProfitAndLossLongTerm ||
																							0,
																					) > 0
																						? 'text-green-600'
																						: Number(
															...
															{(() => {
																const totals =
																	history.lotChangeList.LotChange.reduce(
																		(acc, change) => ({
																			quantityChange:
																				acc.quantityChange +
																				Number(change.quantityChange || 0),
																			quantityFinal:
																				acc.quantityFinal +
																				Number(change.quantityFinal || 0),
																			costBasis:
																				acc.costBasis +
						...
						<Card>
							<CardContent className="flex flex-col items-center justify-center py-12">
								...
								<p className="text-sm text-muted-foreground mt-2">
									No realized profit and loss history records found for this
									PlaidMerge.
								</p>
							</CardContent>
						</Card>
