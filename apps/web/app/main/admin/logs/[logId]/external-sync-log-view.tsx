...
export function ExternalSyncLogView({ data }: ExternalSyncLogViewProps) {
  ...
  const transactionsByType = typedData.investment_transactions?.reduce((acc, txn) => {
  ...
  const securitiesBySector = typedData.securities?.reduce((acc, security) => {
  ...
  return (
    <LogViewerLayout data={data}>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div>
              ...
              <p className="font-medium">{typedData.item?.institution_name || 'N/A'}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="font-medium">{typedData.total_investment_transactions || 0}</p>
            </div>
          </div>
        </CardContent>
      </Card>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="font-mono text-sm">{typedData.item.institution_id}</p>
              </div>
              <div>
                ...
                <tbody>
                  {typedData.accounts.map((account, index) => (
                    <tr key={account.account_id || index} className="border-b">
                      ...
                      <td className="p-2">{account.balances?.iso_currency_code}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {typedData.investment_transactions && typedData.investment_transactions.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle>Investment Transactions ({typedData.investment_transactions.length})</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {typedData.investment_transactions.slice(0, 50).map((transaction, index) => (
                    <tr key={transaction.investment_transaction_id || index} className="border-b">
                      <td className="p-2">
                        {transaction.date 
                      ...
                      <td className="p-2 max-w-xs truncate" title={transaction.name}>
                        {transaction.name}
                      </td>
                      <td className="p-2 font-mono">{transaction.quantity}</td>
                      <td className="p-2 font-mono">${transaction.price}</td>
                      <td className={`p-2 font-mono ${(transaction.amount || 0) >= 0 ? 'text-green-600' : 'text-red-600'}`}>
              ...
              {(typedData.investment_transactions.length > 50) && (
                <p className="text-sm text-muted-foreground mt-2">
                  Showing first 50 transactions of {typedData.investment_transactions.length} total
                </p>
              )}
            </div>
          </CardContent>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {typedData.securities.map((security, index) => (
                    <tr key={security.security_id || index} className="border-b">
                      <td className="p-2 font-mono font-semibold">{security.ticker_symbol}</td>
                      <td className="p-2 max-w-xs truncate" title={security.name}>
                        {security.name}
                      </td>
                      <td className="p-2">
                      ...
                      <td className="p-2 font-mono">
                        {security.close_price ? `$${security.close_price}` : 'N/A'}
                      </td>
                      <td className="p-2">{security.market_identifier_code || 'N/A'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
