...
export default function DeleteAuthConnectionDialog({
  ...
  const [deleteAuthConnection, { loading }] = useMutation(DELETE_AUTH_CONNECTION_MUTATION, {
    onCompleted: () => {
      toast.success(`Connection to ${institutionName || 'institution'} has been removed successfully.`);
      setIsOpen(false);
      if (onSuccess) {
        onSuccess();
      } else {
  ...
  return (
    <AlertDialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <AlertDialogContent className="max-w-2xl">
        <AlertDialogHeader>
          <AlertDialogTitle>Disconnect from {institutionName || 'Institution'}</AlertDialogTitle>
          <AlertDialogDescription className="space-y-4">
            <p>
              Are you sure you want to disconnect from {institutionName || 'this institution'}? 
              This action cannot be undone and will permanently remove:
            </p>
            
            <div className="rounded-lg border bg-muted/30 p-4">
              <h4 className="mb-3 font-medium">
                The following {accountCount} account{accountCount !== 1 ? 's' : ''} will be deleted:
              </h4>
              <ul className="space-y-2">
                {accounts.map(account => (
                  <li key={account.id} className="flex items-center justify-between rounded bg-background p-2">
        ...
        <AlertDialogFooter>
          ...
          <AlertDialogAction
            ...
            disabled={loading}
          >
            {loading ? 'Disconnecting...' : `Disconnect ${accountCount} Account${accountCount !== 1 ? 's' : ''}`}
          </AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
