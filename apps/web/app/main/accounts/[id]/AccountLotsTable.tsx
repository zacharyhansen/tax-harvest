...
import { useAccountLotsQuery } from '~/generated/gql';
...
interface AccountLotsTableProps {
  accountId: string;
}

export default function AccountLotsTable({ accountId }: AccountLotsTableProps) {
  const { data, error, loading } = useAccountLotsQuery({
    variables: { accountId },
  });

  if (loading) {
    return <LoadingPage message="Loading lots..." />;
  }
  ...
  return (
    <DataTable
      columns={columnDef}
      data={data?.lots ?? []}
      noResultsAlert="This account has no lots."
      loading={loading}
      error={!!error}
      initialState={{
        expanded: true,
