import { ArrowUpCircle, TrendingDown, Check } from 'lucide-react';
import { Card } from '@repo/ui/components/card';
import { Button } from '@repo/ui/components/button';

import { Format } from '~/modules/utils';

export interface CostBasisPairCardProps {
  pair: any;
}
...
export function CostBasisPairCard({ pair }: CostBasisPairCardProps) {
  ...
  const calculateDaysHeld = (taxLot: any): number => {
    if (taxLot?.holdingPeriodDays) return taxLot.holdingPeriodDays;
  ...
  return (
    <Card className="overflow-hidden border-0 bg-[#0D0F12] shadow-sm">
      ...
      <div className="divide-border grid gap-0 divide-x bg-[#0D0F12] md:grid-cols-2">
        {/* Gain Position */}
        <div className="p-4">
          ...
          <div className="my-4 grid grid-cols-2 gap-2 text-sm">
            <div>
              ...
              <div className="font-medium">
                {gainOpportunity.taxLot?.quantity || 0} shares
              </div>
            </div>
            <div>
              <div className="text-muted-foreground">Days Held</div>
              <div className="font-medium">{gainDaysHeld} days</div>
            </div>
            <div>
        ...
        <div className="p-4">
          <div className="mb-2 flex items-center justify-between">
            <div className="flex items-center">
              <div className="flex h-8 w-8 items-center justify-center rounded-full bg-red-100 text-xs font-bold text-red-800">
          ...
          <div className="my-4 grid grid-cols-2 gap-2 text-sm">
            <div>
              <div className="text-muted-foreground">Quantity</div>
              <div className="font-medium">
                {lossOpportunity.taxLot?.quantity || 0} shares
              </div>
            </div>
            <div>
              <div className="text-muted-foreground">Days Held</div>
              <div className="font-medium">{lossDaysHeld} days</div>
            </div>
            <div>
              <div className="text-muted-foreground">Purchase Price</div>
              <div className="font-medium">
      ...
      <div className="border-t bg-[#0D0F12] p-4 text-white">
        ...
        <div className="flex justify-center">
          ...
          >
            <Check className="mr-2 h-4 w-4" /> Execute Trade
          </Button>
        </div>
      </div>
    </Card>
