...
export class LotCurrent {
	...
	@Field({
		description: 'How many shares from this lot are in "in flight" harvests.',
	})
	currentHarvestQty: string;

	@Field({
		description: 'How many shares from this lot are available to be harvested.',
	})
	availableQty: string;
...
@ObjectType({
	description: "GQL object for the lot fields returned in harvest API's",
})
export class HarvestLotCurrent extends LotCurrent {
	@Field({
		description: 'How many shares from this lot are we going to harvest.',
	})
