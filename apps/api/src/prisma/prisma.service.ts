...
export class PrismaService extends PrismaClient implements OnModuleInit {
  ...
  static bypassRLS() {
    return Prisma.defineExtension(prisma =>
      prisma.$extends({
        client: {
          $transaction: (async (...txnParams: Parameters<typeof prisma.$transaction>) => {
        ...
        query: {
          $allModels: {
            async $allOperations({ args, query, ...rest }) {
              ...
              if (existingTxn)
                return query(args) // handled via 'client' override
          ...
          async $allOperations({ args, model, query, ...rest }) {
            if (model)
              return query(args) // no override needed... should be handled in $allModels above

            // NOTE: inside a raw SQL operation like like '$queryRaw' or '$executeRaw'
            const existingTxn = get(rest, ['__internalParams', 'transaction'])
            if (existingTxn)
              return query(args) // already handled via client override
        ...
        client: {
          $transaction: (async (...txnParams: Parameters<typeof prisma.$transaction>) => {
            ...
            if (Array.isArray(txnOps)) {
              // Inject as the first step of a batch operation
              const [_rlsHelper, ...result] = await prisma.$transaction(
                [prisma.$executeRaw`SELECT set_config('app.current_portfolio_id', ${portfolioId}::text, TRUE)`, ...txnOps],
                txnOptions,
              )
              return result
            }
