...
describe('portfolioService', () => {
  ...
  describe('matchLots', () => {
    ...
    it('should calculate P&L summary values correctly', () => {
      ...
      expect(result[0][0].sourceHarvestPAndL + result[0][0].matchedHarvestPAndL).toBe(0)
    })
    ...
    describe('minPAndL filtering', () => {
      it('should filter out source lots that do not meet minPAndL threshold', () => {
        ...
        expect(Math.abs(result[0][0].sourceHarvestPAndL)).toBeGreaterThanOrEqual(100)
      })
      ...
      it('should filter out matched results that do not meet minPAndL threshold', () => {
        ...
        expect(Math.abs(result[0][0].matchedHarvestPAndL)).toBeGreaterThanOrEqual(100)
      })
    ...
    describe('zero harvest quantity filtering', () => {
      ...
      it('should include results where all lots have non-zero harvest quantities', () => {
        ...
        expect(result).toHaveLength(1)
        expect(result[0]).toHaveLength(1)
        expect(Number(result[0][0].sourceLots[0].harvestQuantity)).toBeGreaterThan(0)
        expect(Number(result[0][0].matchedLots[0].harvestQuantity)).toBeGreaterThan(0)
      })
    ...
    describe('maxPercentageDifference filtering', () => {
      it('should include results when P&L difference is within threshold', () => {
        ...
        const resultWithoutFilter = PortfolioService.matchLots({ sourceLots, matchingLots })
